...
Válido em **qualquer projeto** que use este pack + `issues` de issues-local.
...
| QA multi-perspectiva | `QA` | não |
| Deploy / PR / go-no-go | `Deploy` | não |
...
| Gate | Após | Efeito |
|---|---|---|
| G1 | Planning | Confirma os Requisitos Funcionais / Intenção e Fecha Planning → abre Design |
| Direção | E (se houve) | Humano escolhe opção antes de Spec |
| G2 | Design | Confirma o spec e Fecha Design → abre Implement |
