...
Adaptada de [mattpocock/skills · wayfinder](https://github.com/mattpocock/skills). Tracker fixo: **issues-local**. Idioma: **pt-BR**. Independência de Issues: **D10** (`WORKFLOW.md` / `decisions.md`).
...
O mapa canônico é um **arquivo Markdown no repo** (ex.: `docs/wayfinding/<slug>.md`), referenciado pela Issue índice em `artifacts`. A Issue índice tem TAG=`Planning` e o mesmo **projeto** das Issues-ticket.
...
Sem labels GitHub (`wayfinder:map`, etc.). Sem grafo de blocking nativo: Issues são **independentes** (D10). Ordem sugerida e fronteira ficam no próprio mapa.
...
<o que chegar ao fim deste mapa significa — spec, decisão ou mudança. 1–2 linhas.>

## Notas

<domínio; skills a consultar; projeto issues-local deste esforço; preferências>

## Decisões até agora

- [<título da Issue fechada>](caminho ou id) — <gist de uma linha da resposta>

## Fronteira sugerida

<!-- Issues OPEN ainda não decididas, na ordem preferida de ataque — sem blocking obrigatório -->

- <título> — <tipo: research | domain | esboço | tarefa>

## Ainda não especificado

<!-- névoa no escopo: perguntas que ainda não dá para ticketar com precisão -->

## Fora de escopo

<!-- trabalho consciente além do destino; não gradua para ticket -->
```
...
|---|---|---|
| `research` | AFK | Skill [`research`](../research/SKILL.md) |
| `domain` | HITL | Skill [`domain-modeling`](../domain-modeling/SKILL.md) — conversa + glossário/ADR |
...
Não há claim por `--id`. Paralelo entre tickets OPEN do mesmo projeto é permitido; espere edição concorrente do mapa.
...
Obtida via [`planning-phase`](../planning-phase/SKILL.md). Não carregue skills de Design/Implement/QA/Deployment neste claim.
