...
export function validateQa(queue: Queue, issue: Issue): void {
  ...
  if (content === null) throw new DomainError(`Issue QA não conclui sem o artefato de validação: registre o resultado requisito×comportamento com: issues artifact --id ${issue.id} --file <qa.md>`);
