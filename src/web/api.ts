...
async function dispatch(request: IncomingMessage, response: ServerResponse, root?: string): Promise<void> {
  ...
  if (request.method === "POST" && route.length === 0) return create(response, body, root);
  if (request.method === "POST" && route[1] === "close") return close(response, route[0], body, root);
  if (request.method === "POST" && route[1] === "decision") return decide(response, route[0], body, root);
  if (request.method === "POST" && route[1] === "reset") return reset(response, route[0], body, root);
...
function list(response: ServerResponse, url: URL, root?: string): void {
  ...
  const issues = new ListIssuesUseCase(root).execute({ status: query.get("status") ?? undefined,
    project: query.get("project") ?? undefined, title: query.get("title") ?? undefined, tag: query.get("tag") ?? undefined,
...
function create(response: ServerResponse, body: Body, root?: string): void {
  const issue = new CreateIssueUseCase(root).execute({ title: text(body, "title"), project: text(body, "project"),
    tag: text(body, "tag"), problem: text(body, "problem"), artifacts: text(body, "artifacts"),
    acceptance_criteria: text(body, "acceptance_criteria"), actor: "human" });
